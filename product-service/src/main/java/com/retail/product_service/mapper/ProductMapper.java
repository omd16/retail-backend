...
@Mapper(componentModel = "spring", uses = {CategoryMapper.class, JsonNullableMappingStrategy.class})
public interface ProductMapper {
    ...
    List<Product> mapList(List<ProductRequestDto> dto, @Context ProductMapperContext productMapperContext);


    @Mapping(target = "id", ignore = true)
    void update(@MappingTarget Product product, ProductRequestDto dto, @Context ProductMapperContext productMapperContext);

    @Mapping(target = "id", ignore = true)
    @BeanMapping(nullValuePropertyMappingStrategy = NullValuePropertyMappingStrategy.IGNORE)
    @Mapping(target = "media", source = "media", qualifiedByName = "patchMediaList")
    void patch(@MappingTarget Product product, ProductRequestDto dto, @Context Product productContext);
