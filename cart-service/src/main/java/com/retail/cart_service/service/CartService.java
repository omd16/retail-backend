...
public class CartService {
    ...
    public CartResponse updateCartProduct(UUID userId, CartItemRequest cartItemRequest){
        var userCart = cartRepository.findByUserId(userId).orElseGet(() -> createNewUserCart(userId, cartItemRequest));

        userCart.getItems().stream()
        .filter(item -> item.getProductId().equals(cartItemRequest.getProductId())).findAny()
                .ifPresentOrElse(item -> updateItem(item, userCart,  cartItemRequest), () -> addItem(userCart, cartItemRequest));
